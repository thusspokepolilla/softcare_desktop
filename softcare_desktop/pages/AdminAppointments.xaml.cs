...
namespace softcare_desktop.pages
{
    ...
    public partial class AdminAppointments : Page
    {
        ...
        private void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            string conString = "server=localhost; user=root; database=softcare_hospital_db";
            MySqlConnection con = new MySqlConnection(conString);
            string delString = "DELETE FROM appointments WHERE appointment_id=@appointment_id";
            MySqlCommand cmdDel = new MySqlCommand(delString, con);
            try
            {
                if (dtgAppointments.SelectedIndex != -1 && tbxAppointmentID.Text != "")
                {
                    con.Open();
                    cmdDel.Parameters.AddWithValue("@appointment_id", tbxAppointmentID.Text);
                    if (MessageBox.Show("Are you sure you want to delete this appointment?", "Confirmation", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
                    {
                        if (cmdDel.ExecuteNonQuery() < 0)
                        {
                            MessageBox.Show("Appointment deletion failed.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        }
                        else
                        {
